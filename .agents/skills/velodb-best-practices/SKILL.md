...
| JOINs are slow / shuffle | `usecase-star-schema-join` | Colocation: same colocate_with group |
...
| Storage growing too fast | `schema-partition-dynamic-ttl` + `schema-props-compression` | Dynamic partition TTL + ZSTD compression |
...
- [ ] **Partition strategy** — Time-series? RANGE. Sporadic? AUTO. Small table? Skip. → `schema-partition-*`
- [ ] **Bucket key** — High cardinality? HASH. No obvious key? RANDOM (DUP only). → `schema-bucket-*`
...
- `schema-keys-prefix-index-limits` — 3 columns, 36 bytes
